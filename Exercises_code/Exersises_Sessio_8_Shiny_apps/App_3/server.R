...
function(input, output, session) {
    ...
    output$distPlot_2 <- renderPlot({
      ...
      bins <- seq(min(selected_data$Sepal.Length), max(selected_data$Sepal.Length), length.out = input$bins_2 + 1)
      
      # draw the histogram with the specified number of bins
      hist(selected_data$Sepal.Length, breaks = bins, col = 'darkblue', border = 'grey',
           xlab = 'Sepal Length of iris virginica',
           main = 'Histogram length')
